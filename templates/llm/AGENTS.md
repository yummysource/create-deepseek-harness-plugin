...
`{{PKG_NAME}}` is a DeepSeek Harness plugin bundle, generated from the `llm` template.
...
`stream()` is the only method you must implement. `providerInfo`, `listModels`, `resolveModel`,
and `providerRetryPolicy` have working defaults — override one only when the provider answers
better than the default.

Two contracts are not optional:

1. **Every HTTP request to the provider carries `attributionHeaders()`.** Merge it into your
   headers; never replace them with your own set. `buildHeaders()` is where this happens, so
   keep it on the path of whatever call you add.
2. **`stream()` honors `options.signal`,** so a cancelled turn stops the provider call instead
   of leaking it.

The chunk order is `block-start → text-delta… → block-end → usage → finish`. `block-end` carries
the COMPLETE block, not just the last delta — a consumer rebuilds the whole message from it.
`finish` takes a `FinishReason` object with a `kind`, not a bare string.

The generated body echoes instead of calling a provider, so the project boots as generated.
Replace the marked line; booting proves registration, not routing.
