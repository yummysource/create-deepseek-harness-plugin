...
`{{PKG_NAME}}` is a DeepSeek Harness plugin bundle, generated from the `service` template.
...
Two roles in one package: the Service Definition (`src/index.ts`) says WHAT the capability does,
the provider (`src/local.ts`) says HOW. Only the provider is a loadable row; the Definition is
imported for types and subclassed. Split them into separate packages only when they need to
evolve independently.

A Service class declares its schema as a STATIC member (`static Config = Schema.object(...)`),
unlike a function plugin which exports `const Config`. Use the wrong form and the constructor
receives an undefined config.

Loading a second implementation of the same service throws. That is cordis' duplicate-service
behaviour, not a bug — a deployment picks one provider.
