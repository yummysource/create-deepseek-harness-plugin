...
`{{PKG_NAME}}` is a DeepSeek Harness plugin bundle, generated from the `basic` template.
...
A side-effect plugin. The interval lives in `Config` so a deployment can retune it; the timer
lives in `ctx.effect()` so a reload does not leak one.
