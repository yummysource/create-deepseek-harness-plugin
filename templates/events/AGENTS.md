...
`{{PKG_NAME}}` is a DeepSeek Harness plugin bundle, generated from the `events` template.
...
Four dispatch modes, and they are not interchangeable:

| Mode | Contract |
|---|---|
| `emit` | Notification. Every listener runs; return values ignored. |
| `bail` | Question. First answer other than null/false/undefined wins; later listeners never run. |
| `serial` | Ordered, async results awaited, stops at the first real answer. |
| `waterfall` | Chain. **MUST call `next()`.** |

Omitting `next()` in a waterfall listener short-circuits the chain. For `tools/pre-execute` the
tool call simply never happens, with no error anywhere — the most expensive mistake in this
file's territory. An observer always delegates; returning a decision is what a POLICY plugin
does, and that belongs in its own plugin.

`turn/*`, `step/*`, `tool/call`, `tool/result`, and `compaction/*` are durable session-event
TYPES, not same-named cordis events. Observe them through `session/event` and inspect
`event.type`.

Declaring events on `interface Events` is how this plugin becomes an extension point for someone
else. Declaration merging types both the listener signature and the dispatch arguments.
