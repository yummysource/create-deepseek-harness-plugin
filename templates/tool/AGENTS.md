...
`{{PKG_NAME}}` is a DeepSeek Harness plugin bundle, generated from the `tool` template.
...
Registering a tool carries obligations beyond making it run:

- Write `description` and parameter descriptions from the MODEL's side — task vocabulary only,
  no UI or transport terms.
- Mark every parameter the model must supply `required: true`.
- `output.schema` describes exactly what `execute` returns; `render` is a pure function of it.
- `presentCall` decides the UI card and is PURE — it runs on live streaming and on session-log
  replay alike, so no I/O, no clock, no random, no session state. A tool's render intent is part
  of its design, not an afterthought.

Booting and seeing `listed=true` proves registration. Having the model actually call the tool
needs `DEEPSEEK_API_KEY`; without one the call fails with `MISSING_CREDENTIAL`.
